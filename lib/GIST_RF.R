...
feature_gist<-read.csv("../lib/gist/gist10.csv")
label<-read.csv("label_train.csv")
train_data<-feature_gist[-test_index,]
train_label<-label[-test_index,]$label
test_data<-feature_gist[test_index,]
...
for(tree in 1:length(n_trees)){
  for(k in 1:K){
    ...
    rf_model <- tuneRF(train.data, as.factor(train.label), ntreeTry = n_trees[tree],plot=FALSE, stepFactor=1.5, improve=1e-5,doBest = TRUE)
    tune_mtry[k]<-rf_model$mtry
    model_pred<-predict(rf_model,validation.data)
    cv_error[k]<-mean(model_pred!=validation.label)
  }
  print(cv_error)
  cv_error_min[tree]<-min(cv_error,na.rm = TRUE)
...
train_error <- mean(rf_model_best$predicted != train_label)#0.288
save(rf_model_best, file="RFmodel_GIST.RData")

test_time<-system.time(pred <- predict(rf_model_best,test_data))
test_error <- mean(pred != test_label)
test_error#0.283
save(test_error, file="RFmodel_GIST_TestError.RData")
