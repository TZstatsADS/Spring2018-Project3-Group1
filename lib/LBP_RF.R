...
test_index<-read.table("../data/TEST-NUMBER.txt")
test_index<-as.vector(t(test_index))
feature_lbp<-read.csv("../lib/lbp/lbp.csv",header=FALSE)
label<-read.csv("../data/label_train.csv")
train_data<-feature_lbp[-test_index,]
train_label<-label[-test_index,]$label
test_data<-feature_lbp[test_index,]
...
train_error <- mean(rf_model_best$predicted != train_label)#0.277
save(rf_model_best, file="RFmodel_LBP.RData")

test_time<-system.time(pred <- predict(rf_model_best,test_data))
test_error <- mean(pred != test_label)
test_error#0.283
save(test_error, file="RFmodel_LBP_TestError.RData")
